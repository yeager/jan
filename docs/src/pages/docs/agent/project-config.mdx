...
# max_retries = 10  # resend attempts after a failed or answerless upstream call; 0 disables retrying
# retry_backoff_ms = 250  # base delay before the first retry; doubles per attempt, capped at 8s, jittered
...
| `max_retries` | `10` | Attempts to resend a failed upstream call, counted *after* the first failure. Covers a dropped or refused connection, a request timeout, HTTP 408/409/425/429 and any 5xx, and a turn that came back with no answer at all. Set `0` to disable retrying - a real setting, distinct from leaving the key unset |
| `retry_backoff_ms` | `250` | Base delay before the first retry. Doubles per attempt, capped at 8s, then jittered down into [0.5x, 1.0x] so many clients do not retry one provider in lockstep. A `Retry-After` header from the upstream wins only when it is longer than the computed delay, and is taken as-is (not jittered, not capped) |
