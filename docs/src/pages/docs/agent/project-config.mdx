...
| `retry_backoff_ms` | `250` | Base delay before the first retry. Doubles per attempt, capped at 8s, then jittered down into [0.5x, 1.0x] so many clients do not retry one provider in lockstep. A `Retry-After` header from the upstream replaces the computed delay |
